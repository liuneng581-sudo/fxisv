...
export default function OpenClawDocPage() {
  ...
  useEffect(() => {
    fetch('/api/tutorials?category=OpenClaw').then(r => r.json()).then(data => {
  ...
  const linkColor = theme === 'dark' ? '#0071e3' : '#0066cc';
  ...
  if (!doc) return <div style={{ backgroundColor: bg, minHeight: '100vh', paddingTop: '44px' }}><div className="max-w-[1024px] mx-auto px-6 py-20 text-center" style={{ color: textColor }}><p>文档不存在</p><Link href="/openclaw" style={{ color: linkColor }}>← 返回 OpenClaw</Link></div></div>;
  return (
    <div style={{ backgroundColor: bg, minHeight: '100vh', paddingTop: '44px' }}>
      <div className="max-w-[1024px] mx-auto px-6 py-10">
        <Link href="/openclaw" className="inline-flex items-center gap-1 text-[13px] mb-8 block" style={{ color: textColor }}>← 返回 OpenClaw 文档</Link>

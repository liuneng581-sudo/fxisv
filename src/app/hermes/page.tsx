...
export default function HermesPage() {
  ...
  useEffect(() => {
    tutorials.list('Hermes').then(res => {
      setDocs(res.list || []);
      setLoading(false);
  ...
  return (
    <div style={{ backgroundColor: '#0a0a0a', minHeight: '100vh', paddingTop: '44px' }}>
      <div className="relative overflow-hidden min-h-[200px]" style={{ borderBottom: '1px solid #2d2d30' }}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}>
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <style>{`
            @keyframes floatHermes {
              0% { transform: translateY(0) scale(1); }
              50% { transform: translateY(-12px) scale(1.05); }
              100% { transform: translateY(0) scale(1); }
            }
          `}</style>
          <img src="/icons/hermes.svg" className="w-[200px] h-[200px]" alt=""
            style={{
              animation: hovered ? 'floatHermes 2.5s ease-in-out infinite' : 'none',
              opacity: hovered ? 0.065 : 0.045,
            }} />
        </div>
        <div className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: 'linear-gradient(90deg, #af52de, #ff6b9d)' }} />
        <div className="relative max-w-[1024px] mx-auto px-6 pt-12 pb-10">
          <p className="text-[12px] font-medium tracking-[0.2em] uppercase mb-3" style={{ color: '#af52de' }}>Hermes</p>
          <h1 className="text-[40px] font-semibold tracking-tight mb-3" style={{ color: '#f5f5f7' }}>Hermes</h1>
          <p className="text-[15px]" style={{ color: '#86868b' }}>下一代多智能体协作框架</p>
        </div>
      </div>
      ...
      <div className="max-w-[1024px] mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? (
            ...
            filtered.map(doc => (
              <Link key={doc.id} href={`/hermes/tutorial/${doc.id}`}
